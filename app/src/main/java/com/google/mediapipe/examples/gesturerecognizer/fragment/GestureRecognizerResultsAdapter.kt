    ...
    inner class ViewHolder(private val binding: ItemGestureRecognizerResultBinding) :
        ...
        fun bind(label: String?, score: Float?) {
            scope.launch {
                ...
                with(binding) {
                    val existingText = tvLabel.text.toString()
                    ...
                    // Process label to remove consecutive duplicates
                    val processedLabel = if (label != null) removeConsecutiveDuplicates(label) else NO_VALUE

                    // Append processed label to existing text, avoiding consecutive duplicates
                    tvLabel.text = if (processedLabel != NO_VALUE) {
                        val updatedText = existingText + processedLabel
                        removeConsecutiveDuplicates(updatedText)
                    } else {
                        NO_VALUE
                    }
